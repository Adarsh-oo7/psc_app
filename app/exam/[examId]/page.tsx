...
import { useRouter, useParams, Stack } from 'expo-router';
...
export default function ExamModeSelectionPage() {
    ...
    // Fetch the specific exam details to display its name
    ...
    return (
        <SafeAreaView style={[styles.container, isDarkMode && styles.containerDark]}>
            <Stack.Screen options={{ title: exam?.name || 'Select Mode' }} />
            ...
            <View style={styles.cardContainer}>
                <ModeCard
                    ...
                    onPress={() => router.push(`/exam/${examId}/study`)}
                    isDarkMode={isDarkMode}
                />
                <ModeCard
                    title="Quiz Mode"
                    description="Take a timed quiz with 15 random questions from this exam. See results at the end."
                    icon="timer-outline"
                    onPress={() => router.push(`/quiz/page?exam_id=${examId}`)}
                    isDarkMode={isDarkMode}
                />
            </View>
        </SafeAreaView>
...
const styles = StyleSheet.create({
    ...
    cardTitle: { fontSize: 22, fontWeight: 'bold', marginTop: 16, marginBottom: 8 },
